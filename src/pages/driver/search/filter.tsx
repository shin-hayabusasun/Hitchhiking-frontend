...
const Switch = ({ checked, onChange }: { checked: boolean | null, onChange: (val: boolean) => void }) => (
  <button
    ...
    className={`w-10 h-5 rounded-full transition-colors relative duration-200 ease-in-out ${checked ? 'bg-green-600' : 'bg-gray-300'}`}
  >
        ...
        <main className="flex-1 overflow-y-auto p-4 space-y-4 pb-28">
          <section className="bg-white p-4 rounded-[1.5rem] shadow-sm space-y-3">
            ...
            <div className="space-y-2">
              <div className="relative">
                <LucideMapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" size={16} />
                <input type="text" placeholder="出発地" className="w-full bg-gray-50 py-3 pl-11 pr-4 rounded-xl text-sm outline-none focus:ring-1 focus:ring-green-500" value={filter.departure} onChange={(e) => setFilter({ ...filter, departure: e.target.value })} />
              </div>
              <div className="relative">
                <LucideMapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-green-500" size={16} />
                <input type="text" placeholder="目的地" className="w-full bg-gray-50 py-3 pl-11 pr-4 rounded-xl text-sm outline-none focus:ring-1 focus:ring-green-500" value={filter.destination} onChange={(e) => setFilter({ ...filter, destination: e.target.value })} />
              </div>
            </div>
          </section>
          ...
          <section className="bg-white p-4 rounded-[1.5rem] shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              <div className="relative">
                <LucideCalendar className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                <input type="date" className="w-full bg-gray-50 py-3 pl-11 pr-4 rounded-xl text-sm outline-none" value={filter.date || ''} onChange={(e) => setFilter({ ...filter, date: e.target.value })} />
              </div>
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 relative">
                    ...
                    <input type="time" className="w-full bg-gray-50 py-2 pl-9 pr-2 rounded-xl text-sm outline-none" value={filter.timeRange?.start || '00:00'} onChange={(e) => updateTimeRange('start', e.target.value)} />
                  </div>
                  <span className="text-gray-300 font-bold text-xs">〜</span>
                  <div className="flex-1 relative">
                    <LucideClock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} />
                    <input type="time" className="w-full bg-gray-50 py-2 pl-9 pr-2 rounded-xl text-sm outline-none" value={filter.timeRange?.end || '23:59'} onChange={(e) => updateTimeRange('end', e.target.value)} />
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-4 gap-2">
                {['すべて', '朝', '昼', '夜'].map((label) => (
                  <button key={label} onClick={() => setTimePreset(label)} className={`py-1.5 rounded-xl text-xs font-bold border transition-all duration-200 ${((label === 'すべて' && filter.timeRange?.start === '00:00' && filter.timeRange?.end === '23:59') || (label === '朝' && filter.timeRange?.start === '05:00') || (label === '昼' && filter.timeRange?.start === '12:00') || (label === '夜' && filter.timeRange?.start === '18:00')) ? 'bg-black text-white border-black' : 'bg-white border-gray-100 text-gray-800'}`}>{label}</button>
                ))}
              </div>
            </div>
          </section>
          ...
          <section className="bg-white p-4 rounded-[1.5rem] shadow-sm space-y-3">
            <div className="flex justify-between items-center">
              ...
              <div className="text-green-600 text-[10px] font-bold bg-green-50 px-2 py-0.5 rounded-full">¥{filter.priceRange.min || 0} 以上</div>
            </div>
            <div className="px-2 py-2">
              <input type="range" min="0" max="10000" step="500" className="w-full h-1.5 bg-gray-100 rounded-lg appearance-none cursor-pointer accent-green-600" value={filter.priceRange.min || 0} onChange={(e) => setFilter({ ...filter, priceRange: { ...filter.priceRange, min: parseInt(e.target.value) } })} />
            </div>
          </section>
          ...
          <section className="bg-white p-4 rounded-[1.5rem] shadow-sm space-y-3">
            ...
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-gray-50 rounded-xl text-gray-400"><LucideUsers size={18} /></div>
              <div className="flex gap-2 flex-1">
                {[1, 2, 3, 4].map((num) => (
                  <button key={num} onClick={() => setFilter({ ...filter, seats: num })} className={`flex-1 py-2.5 rounded-xl text-xs font-bold border transition-all duration-200 ${filter.seats === num ? 'bg-black text-white border-black' : 'bg-white text-gray-800 border-gray-100'}`}>{num}人</button>
                ))}
              </div>
            </div>
          </section>
